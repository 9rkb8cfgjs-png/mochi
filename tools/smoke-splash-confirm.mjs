...
await sleep(400);
const st1 = await evalJs("(function(){var c=document.getElementById('splash-confirm');var s=document.getElementById('splash');var btn=c.querySelector('#splash-confirm-ok');var card=c.querySelector('.splash-confirm-card');var top=c.querySelector('.splash-confirm-top');var br=btn.getBoundingClientRect(), cr=card.getBoundingClientRect();return JSON.stringify({confirmVisible:!c.hidden,confirmText:(c.querySelector('.splash-confirm-text')||{}).textContent||'',topText:(top||{}).textContent||'',btnText:btn.textContent.trim(),btnFits:br.left>=cr.left-1&&br.right<=cr.right+1,splashStillThere:!s.classList.contains('hide')});})()") || '{}';
...
check('文案含「简单说明」段落', (st1j.confirmText || '').indexOf('简单说明') >= 0 && (st1j.confirmText || '').indexOf('特定机型') >= 0);
check('文案含「详细说明」段落', (st1j.confirmText || '').indexOf('详细说明') >= 0 && (st1j.confirmText || '').indexOf('设备兼容') >= 0);
check('文案含「要修得先知道」+ 后台消息提醒', (st1j.confirmText || '').indexOf('要修得先知道') >= 0 && (st1j.confirmText || '').indexOf('后台的消息堆的非常多') >= 0);
check('文案含「关于bug」标题', (st1j.confirmText || '').indexOf('关于bug') >= 0);
